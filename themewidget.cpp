	...
	m_console(new Console)
{
    ...
    chartView = new QChartView(createTemperture1Chart());
    m_ui->gridLayout->addWidget(chartView, 1, 1);
    m_charts << chartView;

    chartView = new QChartView(createTemperture2Chart());
    m_ui->gridLayout->addWidget(chartView, 2, 1);
    m_charts << chartView;

	chartView = new QChartView(createPressureChart());
	m_ui->gridLayout->addWidget(chartView, 3, 1);
	m_charts << chartView;

	m_ui->gridLayout->addWidget(m_console, 4, 1);
	...
	connect(this->m_ui->btnSETTING, SIGNAL(clicked()), this, SLOT(btnSETTINGHandler()));
...
QChart * ThemeWidget::createTemperture1Chart() const
{
	QChart *chart = new QChart();
	chart->setTitle("Temperture(1) chart");
	QString name("Series ");
	int nameIndex = 0;
	for (const DataList &list : m_dataTable) {
		QSplineSeries *series = new QSplineSeries(chart);
...
QChart * ThemeWidget::createTemperture2Chart() const
{
	QChart *chart = new QChart();
	chart->setTitle("Temperture(2) chart");
	QString name("Series ");
	int nameIndex = 0;
	for (const DataList &list : m_dataTable) {
		QSplineSeries *series = new QSplineSeries(chart);
...
QChart * ThemeWidget::createPressureChart() const
{
	QChart *chart = new QChart();
...
void ThemeWidget::btnSETTINGHandler()
{
